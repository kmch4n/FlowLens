...
from PySide6.QtWidgets import QLabel, QListView, QScrollBar, QVBoxLayout, QWidget
...
class TranscriptView(QWidget):
    """Render committed transcript rows and one ephemeral partial row."""
    ...
    def return_to_latest(self) -> None:
        ...
        QTimer.singleShot(0, self._scroll_to_maximum)
    ...
    def rendered_text(self) -> str:
        ...
        if self.partial_label.text():
            rows.append(self.partial_label.text())
        return "\n".join(rows)
    ...
    def sync_partials(self) -> None:
        ...
        for source in (AudioSource.ME, AudioSource.OTHERS):
            ...
            if partial is not None:
                ...
                self.partial_label.setText(f"{source_label} · Partial · {partial.text}")
                self.partial_label.setAccessibleDescription(
                    f"Partial transcript from {source.value}"
                )
                self.partial_label.show()
                return
        self.partial_label.clear()
        self.partial_label.setAccessibleDescription("No partial transcript")
    ...
    def _configure(self) -> None:
        ...
        self.partial_label.setProperty("flowlensTone", "muted")
