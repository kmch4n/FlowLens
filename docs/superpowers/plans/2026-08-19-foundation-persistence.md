...
Require an aware `recovered_at`. In deterministic filename order: inspect all artifacts without mutation; build the retained event prefix plus one `SESSION_RECOVERED` recovery-intent event sourced from `GUI` at the next event sequence in a sibling temporary file; fully write, flush, and fsync it; then atomically publish it before any destructive repair, for both clean and torn event tails. Apply each approved JSONL tail repair; repair both WAV headers; atomically rebuild `discussion-state.json` only when final state history is newer; and re-parse all retained records. While the verified directory anchors remain held and before replacing the manifest, require the exact seven-artifact set, exact equality with the retained typed transcript/state/base-event tuples, exact PCM payload digests, exact reconstructed pause intervals, and the expected final snapshot and recovery intent. Build recovery notes naming discarded byte counts and repaired WAV header byte counts, then atomically replace `session.json` last with status `RECOVERED`, `ended_at=recovered_at`, recovered transcript count/final state revision, pause intervals reconstructed from valid `PAUSE_START`/`PAUSE_END` pairs, and `active_duration_ms` derived from the larger valid source sample count at 16,000 Hz. Event details are exactly `{"discarded_jsonl_tail_bytes": {name: count}, "repaired_wav_headers": [sorted names]}` and contain no transcript text or PCM. An unmatched final `PAUSE_START` becomes a pause ending at the recovered active-duration boundary and is named in recovery notes. If inspection finds a valid final `SESSION_RECOVERED` event while the manifest remains `incomplete`, treat it as an interrupted recovery transaction: reuse that intent event's timestamp/details, do not append a second event, idempotently apply or verify every still-pending repair named by the intent, re-parse, and then retry the final recovered-manifest replacement. Any intent temporary write, flush, fsync, close, or replace failure occurs before repair and leaves the original event bytes unchanged. Every atomic session, discussion, and event publication passes the expected target identity into the anchored replace operation, which performs the final type, link-count, identity, size, timestamp, and content-hash validation immediately before the operating-system replace.
