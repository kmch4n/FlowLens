...
def run_audio_worker(
    ...
    asr_audio_out: Queue[AudioFrame | AudioDrainFence],
) -> None: ...

def run_asr_worker(
    config: AsrWorkerConfig,
    audio_in: Queue[AudioFrame | AudioDrainFence],
    control_in: Queue[MessageEnvelope[object]],
    control_out: Queue[MessageEnvelope[object]],
) -> None: ...
```
...
Control messages use shared `WORKER_START`, `WORKER_PAUSE`, `WORKER_RESUME`, and `WORKER_STOP` with a typed payload naming the target worker. Drain acknowledgements use `WORKER_STOPPED`: Audio reports `writer_frames` and `asr_frames`, ASR reports `committed_count`, and Discussion reports `final_revision` and `pending_count`, with every payload also carrying `worker` and `drained=true`. Audio `WORKER_STOPPED/drained=true` specifically means Audio has stopped capture, put every final `AudioWriteCommand`, put exactly one shared `AudioDrainFence` on the dedicated Writer audio queue, and put the stop-boundary fence after all preceding frames on the dedicated ASR queue. Each pause also produces one ordered ASR fence. Runtime status uses shared `AUDIO_LEVEL`, `SOURCE_DISCONNECTED`, `SOURCE_RECONNECTED`, `ASR_STATUS`, and `DISCUSSION_STATUS`. `ASR_STATUS` is authoritative and carries `state`, current `backlog_ms`, cumulative `maximum_backlog_ms`, and `analysis_paused`; the cumulative maximum is updated on every ASR poll even though statuses are emitted only on transitions and at final stop. A discussion failure is a `DISCUSSION_STATUS` runtime message and a persisted `EventType.ANALYSIS_FAILED`; there is no separate failure-only message type.
...
Route `AudioWriteCommand` only on the dedicated Writer-audio queue. Route
`AudioDrainFence` only on the dedicated Writer- or ASR-audio queue appropriate
to its boundary; neither is a general control envelope. General envelopes stay
small. The controller validates each sender-local input sequence, but it
...
after every Writer audio command and its separate ASR fence was enqueued after
every ASR frame. ASR stop uses `{"worker": "ASR", "finalize": true}` and cannot
finalize until it consumes that ordered ASR fence. Discussion stop uses
`{"worker": "DISCUSSION", "finalize": true}` only after the ASR
`WORKER_STOPPED` drain acknowledgement. The controller may send
...
- `AudioQueueBindings(writer_audio_out, asr_audio_out)` exposes the dedicated `Queue[AudioWriteCommand | AudioDrainFence]` Writer path and `Queue[AudioFrame | AudioDrainFence]` ASR path passed only to Audio, Writer, and ASR process arguments.
